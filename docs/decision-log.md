...
The first extraction prompt repeated the full domain schema and trusted metadata for every field, causing Gemma 4 31B requests to exceed the bounded request deadline. Extraction prompt v2 keeps all 21 typed field values and evidence attributes but omits provider-controlled source identity and review state. FundLens injects the parser-derived document hash and pending review state locally, then validates the unchanged full domain model and exact page citations. The request uses provider JSON mode for syntactic validity and explicitly requires collection values inside one field-level evidence envelope. This reduces provider work without weakening evidence traceability or the single-repair rule.
